...
export default (models) => {
  const { utilisateur, verification_email } = models;

  // Initialize service
  const verificationEmailService = VerificationEmailService(models, transporter);

  // Helper to check verification status
  const isVerificationPending = async (userId) => {
    const count = await verification_email.count({ where: { id_utilisateur: userId } });
    return count > 0;
  };
  ...
  return {
    ...
    create: async (req, res, next) => {
      try {
        ...
        // Trigger email verification (create record and send email)
        try {
          await verificationEmailService.create({ id_utilisateur: user.id });
        } catch (emailErr) {
          console.error('Failed to initiate email verification:', emailErr);
          // Proceed without failing the request; user will be "verified" (no record) unless record created before error?
          // If record created, they are unverified. If not, they are verified. 
